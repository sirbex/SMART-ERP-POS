  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      try {
        ...
        const [customerList, transactions] = await Promise.all([
          POSServiceAPI.getCustomersForPOS(),
          POSServiceAPI.getRecentTransactions()
        ]);
        
        // Load inventory from unified endpoint
        ...
        // Transform unified data to InventoryItem format
        const items: InventoryItem[] = unifiedData.map((item: any) => ({
          id: item.id.toString(),
          name: item.name,
          sku: item.sku,
