  ...
  try {
    ...
    if (existingInventory) {
      try {
        ...
        console.error('Error parsing existing inventory:', e);
        // Continue with empty array
      }
    }
    
    // Ensure sampleInventoryWithUoM is an array
    const safeSampleInventory = Array.isArray(sampleInventoryWithUoM) ? sampleInventoryWithUoM : [];
    
    // Check if sample items already exist (to avoid duplicates)
    const sampleIds = safeSampleInventory.map(item => item?.id).filter(Boolean);
    const existingIds = Array.isArray(currentItems) ? 
      currentItems.map(item => item?.id).filter(Boolean) : [];
    ...
    const updatedInventory = [...currentItems, ...safeSampleInventory];
...
export function removeSampleInventoryWithUoM(): void {
  try {
    ...
    const sampleIds = Array.isArray(sampleInventoryWithUoM) ? 
      sampleInventoryWithUoM.map(item => item?.id).filter(Boolean) : [];
    
    // Filter out sample items
    const filteredItems = Array.isArray(currentItems) ? 
      currentItems.filter(item => item && item.id && !sampleIds.includes(item.id)) : [];
    
    localStorage.setItem(STORAGE_KEYS.INVENTORY, JSON.stringify(filteredItems));
