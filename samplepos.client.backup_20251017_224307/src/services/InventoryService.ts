...
import { STORAGE_KEYS } from './UnifiedDataService';
...
export function getInventoryProducts(): InventoryItem[] {
  try {
    const data = localStorage.getItem(STORAGE_KEYS.INVENTORY_PRODUCTS);
    return data ? JSON.parse(data) : [];
  } catch (error) {
    console.error('Failed to get inventory:', error);
    return [];
...
/**
 * Get inventory products with real-time quantities from both systems
 * This function bridges the legacy inventory_items and the new batch system
 */
export function getInventoryWithQuantities(): InventoryItem[] {
  try {
    // GET YOUR REAL INVENTORY FIRST
    let inventory = getInventory();
    
    // If no real inventory, try batch system
    if (inventory.length === 0) {
      inventory = getInventoryProducts();
    }
    
    console.log('� YOUR REAL INVENTORY:', inventory.map(i => ({ name: i.name, qty: i.quantity, price: i.price })));
    
    // Only create sample if you have NO inventory at all
    if (inventory.length === 0) {
      console.log('⚠️ No inventory found - would you like to add items to inventory first?');
      return [];
    }
...
/**
 * Create sample inventory data for demonstration - FORCE CLEAR AND RECREATE
 */
/*
function createSampleInventory(): InventoryItem[] {
  ...
  localStorage.setItem('inventory_items', JSON.stringify(sampleData));
  console.log('✅ FORCED inventory creation:', sampleData.length, 'items');
