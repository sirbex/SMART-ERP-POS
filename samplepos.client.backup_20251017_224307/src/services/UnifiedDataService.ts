...
export interface SaleItem {
  ...
  costPrice?: number; // Cost per unit for this item
  itemCost?: number; // Total cost for this line item (costPrice × quantity)
  itemProfit?: number; // Profit for this line item (itemTotal - itemCost)
}
...
export interface SaleRecord {
  ...
  grossProfit?: number; // Profit before expenses (total - totalCost)
  profitMargin?: number; // Profit percentage ((total - totalCost) / total * 100)
}
...
export class TransactionService {
  ...
  static getTotalCashReceived(transactions: SaleRecord[] = this.getAll()): number {
  ...
  static getTotalCostOfGoodsSold(transactions: SaleRecord[] = this.getAll()): number {
    return transactions.reduce((sum, t) => {
      if (t.totalCost !== undefined) {
        return sum + t.totalCost;
      }
      // Estimate cost for legacy transactions without cost data
      const estimatedCost = t.total * 0.6; // 60% cost assumption
      return sum + estimatedCost;
    }, 0);
  }
  
  static getTotalGrossProfit(transactions: SaleRecord[] = this.getAll()): number {
    return transactions.reduce((sum, t) => {
      if (t.grossProfit !== undefined) {
        return sum + t.grossProfit;
      }
      // Calculate on-the-fly for legacy transactions without profit data
      const estimatedCost = t.totalCost || (t.total * 0.6); // 60% cost assumption
      const calculatedProfit = t.total - estimatedCost;
      return sum + calculatedProfit;
    }, 0);
  }
  
  static getAverageProfitMargin(transactions: SaleRecord[] = this.getAll()): number {
    const validTransactions = transactions.filter(t => t.profitMargin !== undefined && t.total > 0);
    if (validTransactions.length === 0) return 0;
    
    const totalRevenue = validTransactions.reduce((sum, t) => sum + t.total, 0);
    const weightedMarginSum = validTransactions.reduce((sum, t) => 
      sum + (t.profitMargin! * t.total), 0);
    
    return totalRevenue > 0 ? weightedMarginSum / totalRevenue : 0;
  }
}
...
export class CustomerService {
  static getAll(): Customer[] {
    // Use single customer storage location
    const customers = StorageService.get('customers', []);
    console.log(`👥 CustomerService loaded ${customers.length} customers from 'customers' location`);
    return customers;
  }

  static save(customers: Customer[]): boolean {
    console.log(`� CustomerService saving ${customers.length} customers to 'customers' location`);
    return StorageService.set('customers', customers);
  }
