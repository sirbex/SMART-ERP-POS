...
async function migrateToProductUoM() {
  try {
    ...
    // Create or get a general category for miscellaneous units
    let generalCategory = await prisma.uoMCategory.findFirst({
      where: { name: 'General' }
    });

    if (!generalCategory) {
      console.log('📁 Creating General category for units...');
      generalCategory = await prisma.uoMCategory.create({
        data: {
          name: 'General',
          description: 'General units for various product types'
        }
      });
    }
    ...
    for (const unitName of unitsNeeded) {
      ...
      if (!uom) {
        ...
        // Determine if this should be a base unit (typically single items like btl, pcs)
        const isBase = ['btl', 'pcs', 'pc', 'piece', 'bottle', 'item'].includes(unitName.toLowerCase());
        ...
        uom = await prisma.unitOfMeasure.create({
          data: {
            ...
            abbreviation: unitName,
            conversionFactor: 1, // All units here are treated as base (conversion happens in ProductUoM)
            isBase: isBase,
