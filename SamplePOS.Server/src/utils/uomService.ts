...
/**
 * Calculate price for given UoM and quantity with high precision
 * @param basePrice - Base price per base unit (product.sellingPrice)
 * @param quantity - Quantity in the given UoM
 * @param uomId - UoM ID
 * @param product - Product with UoM associations
 * @returns Price calculation result with accurate decimal precision
 */
export async function calculatePriceForUoM(
  ...
  if (productUoM.unitPrice !== null && productUoM.unitPrice !== undefined) {
    // Use the manually set price for this UoM (already stored as Decimal in DB)
    unitPrice = new Decimal(productUoM.unitPrice);
    
    // Validate manual price
    if (unitPrice.lt(0)) {
      throw new Error('Manual unit price cannot be negative');
    }
  } else {
    // Fallback to calculated price using multiplier
    // Unit price = (base price per base unit) × conversion factor × price multiplier
    // All multiplications use Decimal for precision
    const conversionFactor = new Decimal(productUoM.conversionFactor);
    const priceMultiplier = new Decimal(productUoM.priceMultiplier);
    
    unitPrice = price.mul(conversionFactor).mul(priceMultiplier);
  }
  ...
  return {
    unitPrice,
    total,
    priceMultiplier: new Decimal(productUoM.priceMultiplier),
    basePrice: price,
  };
}
