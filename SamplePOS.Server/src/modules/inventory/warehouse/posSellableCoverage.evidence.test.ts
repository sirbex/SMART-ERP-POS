 ...
 * Run: npx vitest run src/modules/inventory/warehouse/posSellableCoverage.evidence.test.ts
 */
...
describe('EVIDENCE — INV-POS sellable coverage SSOT', () => {
  it('guards enable, receive, UI defaults, and coupling', () => {
    const receipt = read('src/modules/inventory/warehouse/multistoreReceiptStore.ts');
    const lotSvc = read('src/modules/inventory-lot/lotService.ts');
    const grn = read('src/modules/inventory/warehouse/warehouseGrnService.ts');
    const settings = read('src/modules/system-settings/systemSettingsService.ts');
    const backfill = read('src/modules/inventory/warehouse/multistoreSellableBackfillService.ts');
    const coverage = read('src/modules/inventory/warehouse/posSellableCoverage.ts');
    const coupling = read('src/services/warehouseInventoryCoupling.ts');
    const stockUi = read(
      path.join('..', 'samplepos.client/src/pages/inventory/StockLevelsPage.tsx'),
    );
    const grUi = read(
      path.join('..', 'samplepos.client/src/pages/inventory/GoodsReceiptsPage.tsx'),
    );
    ...
    gate(
      ...
      'INV-POS gap detector + projection assert exist',
    );
    gate(
      'COUPLING_WIRES_POS',
