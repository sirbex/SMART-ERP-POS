...
export const reportsController = {
    ...
    try {
      ...
      if (params.format === 'pdf') {
        ...
        const columns: PDFTableColumn[] = [
          ...
          { header: 'Status', key: 'status', width: 0.14, format: (v) => v || 'N/A' },
        ];
  ...
  async getGoodsReceived(req: Request, res: Response, pool: Pool) {
    try {
      ...
      if (params.format === 'pdf') {
        ...
        const totalItems = report.data.reduce((sum: number, r: any) => sum + (r.itemsCount || 0), 0);
        const uniqueSuppliers = new Set(report.data.map((r: any) => r.supplierName)).size;
    ...
    try {
      ...
      if (params.format === 'pdf') {
        ...
        const columns: PDFTableColumn[] = [
          ...
          { header: 'Margin %', key: 'grossProfitMargin', width: 0.20, align: 'right', format: (v) => (v !== undefined && v !== null ? v.toFixed(2) + '%' : '0%') },
        ];
    ...
    try {
      ...
      if (params.format === 'pdf') {
        ...
        const columns: PDFTableColumn[] = [
          ...
          { header: 'Total', key: 'totalAmount', width: 0.15, align: 'right', format: (v: any) => formatCurrencyPDF(v) },
          { header: 'Paid', key: 'amountPaid', width: 0.15, align: 'right', format: (v: any) => formatCurrencyPDF(v) },
          { header: 'Balance', key: 'balanceDue', width: 0.15, align: 'right', format: (v: any) => formatCurrencyPDF(v) },
    ...
    try {
      ...
      if (params.format === 'pdf') {
        ...
        const columns: PDFTableColumn[] = [
          ...
          { header: 'Margin %', key: 'profitMarginPercent', width: 0.16, align: 'right', format: (v) => (v !== undefined && v !== null && !isNaN(v) ? `${Number(v).toFixed(1)}%` : '0%') },
        ];
    ...
    try {
      ...
      if (params.format === 'pdf') {
        ...
        const columns: PDFTableColumn[] = [
          ...
          { header: 'Margin', key: 'profitMargin', width: 0.12, align: 'right', format: (v) => (v !== undefined && v !== null ? v.toFixed(1) + '%' : '0%') },
        ];
  ...
  async getCustomerAging(req: Request, res: Response, pool: Pool) {
    try {
      ...
      logger.error('Failed to generate customer aging report', { error: error.message });
      res.status(error.name === 'ZodError' ? 400 : 500).json({
        success: false,
        error: error.name === 'ZodError' ? 'Invalid parameters' : 'Failed to generate report',
        details: error.name === 'ZodError' ? error.errors : undefined,
    ...
    try {
      ...
      if (params.format === 'pdf') {
        ...
        const columns: PDFTableColumn[] = [
          ...
          { header: 'Revenue', key: 'totalRevenue', width: 0.13, align: 'right', format: (v: any) => formatCurrencyPDF(v) },
          { header: 'Cost', key: 'totalCost', width: 0.12, align: 'right', format: (v: any) => formatCurrencyPDF(v) },
          { header: 'Gross Profit', key: 'grossProfit', width: 0.12, align: 'right', format: (v: any) => formatCurrencyPDF(v) },
          { header: 'Margin %', key: 'profitMargin', width: 0.09, align: 'right', format: (v: any) => String(v) + '%' },
          { header: 'Trans.', key: 'transactionCount', width: 0.08, align: 'right' },
          { header: 'Avg Trans.', key: 'averageTransactionValue', width: 0.06, align: 'right', format: (v: any) => formatCurrencyPDF(v) },
        ];
      ...
      logger.error('Failed to generate sales by category report', { error: error.message });
      res.status(error.name === 'ZodError' ? 400 : 500).json({
        success: false,
        error: error.name === 'ZodError' ? 'Invalid parameters' : 'Failed to generate report',
        details: error.name === 'ZodError' ? error.errors : undefined,
    ...
    try {
      ...
      if (params.format === 'pdf') {
        ...
        const columns: PDFTableColumn[] = [
          ...
          { header: 'Transactions', key: 'transactionCount', width: 0.15, align: 'right' },
          { header: 'Total Revenue', key: 'totalRevenue', width: 0.20, align: 'right', format: (v: any) => formatCurrencyPDF(v) },
          { header: 'Avg Amount', key: 'averageAmount', width: 0.20, align: 'right', format: (v: any) => formatCurrencyPDF(v) },
          { header: '% of Total', key: 'percentageOfTotal', width: 0.15, align: 'right', format: (v: any) => String(v) + '%' },
        ];
      ...
      logger.error('Failed to generate sales by payment method report', { error: error.message });
      res.status(error.name === 'ZodError' ? 400 : 500).json({
        success: false,
        error: error.name === 'ZodError' ? 'Invalid parameters' : 'Failed to generate report',
        details: error.name === 'ZodError' ? error.errors : undefined,
  ...
  async getHourlySalesAnalysis(req: Request, res: Response, pool: Pool) {
    try {
      ...
      logger.error('Failed to generate hourly sales analysis report', { error: error.message });
      res.status(error.name === 'ZodError' ? 400 : 500).json({
        success: false,
        error: error.name === 'ZodError' ? 'Invalid parameters' : 'Failed to generate report',
        details: error.name === 'ZodError' ? error.errors : undefined,
  ...
  async getSalesComparison(req: Request, res: Response, pool: Pool) {
    try {
      ...
      logger.error('Failed to generate sales comparison report', { error: error.message });
      res.status(error.name === 'ZodError' ? 400 : 500).json({
        success: false,
        error: error.name === 'ZodError' ? 'Invalid parameters' : 'Failed to generate report',
        details: error.name === 'ZodError' ? error.errors : undefined,
  ...
  async getCustomerPurchaseHistory(req: Request, res: Response, pool: Pool) {
    try {
      ...
      logger.error('Failed to generate customer purchase history report', { error: error.message });
      res.status(error.name === 'ZodError' ? 400 : 500).json({
        success: false,
        error: error.name === 'ZodError' ? 'Invalid parameters' : 'Failed to generate report',
        details: error.name === 'ZodError' ? error.errors : undefined,
  ...
  async getSalesSummaryByDateReport(req: Request, res: Response, pool: Pool) {
    try {
      ...
      const summary = result.length > 0 ? {
        totalRevenue: result.reduce((sum: Decimal, item: any) => sum.plus(item.totalRevenue || 0), new Decimal(0)).toDecimalPlaces(2).toNumber(),
        totalProfit: result.reduce((sum: Decimal, item: any) => sum.plus(item.totalProfit || 0), new Decimal(0)).toDecimalPlaces(2).toNumber(),
        totalTransactions: result.reduce((sum: number, item: any) => sum + parseInt(item.transactionCount || 0, 10), 0),
        averageRevenue: result.length > 0
          ? result.reduce((sum: Decimal, item: any) => sum.plus(item.totalRevenue || 0), new Decimal(0)).dividedBy(result.length).toDecimalPlaces(2).toNumber()
          : 0,
  ...
  async getSalesDetailsReport(req: Request, res: Response, pool: Pool) {
    try {
      ...
      const summary = result.length > 0 ? {
        totalQuantity: result.reduce((sum: Decimal, item: any) => sum.plus(item.total_quantity || 0), new Decimal(0)).toDecimalPlaces(3).toNumber(),
        totalRevenue: result.reduce((sum: Decimal, item: any) => sum.plus(item.total_revenue || 0), new Decimal(0)).toDecimalPlaces(2).toNumber(),
        avgProfitMargin: result.length > 0
          ? result.reduce((sum: Decimal, item: any) => sum.plus(item.profit_margin_percent || 0), new Decimal(0)).dividedBy(result.length).toDecimalPlaces(2).toNumber() + '%'
          : '0%',
        uniqueProducts: new Set(result.map((item: any) => item.product_name)).size,
        transactionCount: result.reduce((sum: number, item: any) => sum + parseInt(item.transaction_count || 0, 10), 0),
      } : {};
      ...
      if (format === 'pdf') {
        ...
        pdfGen.addHeader({
          ...
          subtitle: `Product Sales by Date - ${filters.startDate ? formatDatePDF(filters.startDate) : 'All'} to ${filters.endDate ? formatDatePDF(filters.endDate) : 'All'}`,
          generatedAt: formatDateTime(),
        });
  ...
  async getSalesByCashierReport(req: Request, res: Response, pool: Pool) {
    try {
      ...
      const summary = result.length > 0 ? {
        totalTransactions: result.reduce((sum: number, item: any) => sum + parseInt(item.totalTransactions || 0, 10), 0),
        totalRevenue: result.reduce((sum: Decimal, item: any) => sum.plus(item.totalRevenue || 0), new Decimal(0)).toDecimalPlaces(2).toNumber(),
        totalProfit: result.reduce((sum: Decimal, item: any) => sum.plus(item.totalProfit || 0), new Decimal(0)).toDecimalPlaces(2).toNumber(),
        averageTransactionValue: result.length > 0
          ? result.reduce((sum: Decimal, item: any) => sum.plus(item.avgTransactionValue || 0), new Decimal(0)).dividedBy(result.length).toDecimalPlaces(2).toNumber()
          : 0,
