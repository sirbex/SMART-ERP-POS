...
router.get('/export/pdf', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const totalItems = purchaseOrders.reduce((sum: number, po: any) => sum + (po._count?.items || 0), 0);
    const grandSubtotal = purchaseOrders.reduce((sum: number, po: any) => sum + Number(po.subtotal || 0), 0);
    const grandTax = purchaseOrders.reduce((sum: number, po: any) => sum + Number(po.taxAmount || 0), 0);
    const grandTotal = purchaseOrders.reduce((sum: number, po: any) => sum + Number(po.totalAmount || 0), 0);
    ...
    sendTablePdf(
      ...
      {
        summarySection: [
          { label: 'Total Purchase Orders:', value: totalPOs, color: '#1e40af' },
          { label: 'Total Items:', value: totalItems, color: '#1e40af' },
          { label: 'Grand Subtotal:', value: `₱${grandSubtotal.toFixed(2)}`, color: '#059669' },
          { label: 'Total Tax:', value: `₱${grandTax.toFixed(2)}`, color: '#059669' },
          { label: 'Grand Total:', value: `₱${grandTotal.toFixed(2)}`, color: '#dc2626' }
        ]
      }
    );
  } catch (error: any) {
...
router.get('/:id/items/export/pdf', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    // Add totals row
    const totalOrdered = (po.items as any[]).reduce((sum: number, item: any) => sum + Number(item.orderedQuantity || 0), 0);
    const totalReceived = (po.items as any[]).reduce((sum: number, item: any) => sum + Number(item.receivedQuantity || 0), 0);
    const grandTotal = (po.items as any[]).reduce((sum: number, item: any) => 
      sum + (Number(item.orderedQuantity || 0) * Number(item.unitPrice || 0)), 0);
