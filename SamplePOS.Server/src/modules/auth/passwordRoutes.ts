 ...
 */
router.get('/expiry', authenticate, async (req: Request, res: Response) => {
    try {
        ...
        logger.error('Failed to check password expiry', { error, userId: req.user?.id });
        res.status(500).json({
            success: false,
 ...
 */
router.post('/change', authenticate, strictRateLimit, async (req: Request, res: Response) => {
    try {
        const userId = req.user!.id;
 ...
 */
router.get('/must-change', authenticate, async (req: Request, res: Response) => {
    try {
        const userId = req.user!.id;
        ...
        logger.error('Failed to check must-change status', { error, userId: req.user?.id });
        res.status(500).json({
            success: false,
