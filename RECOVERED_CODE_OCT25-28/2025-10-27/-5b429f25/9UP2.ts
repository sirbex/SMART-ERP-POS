...
router.get('/products/:productId/uoms', authenticate, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // Get full ProductUoM data with nested uom object for management UI
    const productUoMs = await prisma.productUoM.findMany({
      where: {
        productId,
        ...(saleOnly === 'true' ? { isSaleAllowed: true } : {}),
      },
      include: {
        uom: {
          select: {
    ...
    logger.info(`Retrieved ${productUoMs.length} UoMs for product`, {
      productId,
      userId: (req as any).user?.id,
    });

    res.json(productUoMs);
  } catch (error) {
    next(error);
  }
});
