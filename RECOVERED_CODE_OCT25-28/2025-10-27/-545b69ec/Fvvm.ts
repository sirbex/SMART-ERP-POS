...
export async function createTransaction(transaction: {
  ...
  customerId?: string;
  // New: multi-pay support
  payments?: Array<{ method: string; amount: number; reference?: string }>;
  // Back-compat single payment fields (will be adapted to payments array)
  ...
  try {
    ...
    const saleData = {
      ...
      items: transaction.items.map(item => ({
        productId: item.productId?.toString() || '', // Backend expects string
      ...
      // Prefer provided payments array; else adapt from single payment fields
      payments: (transaction.payments && transaction.payments.length > 0)
        ...
        : [{
            method: (transaction.paymentMethod || 'CASH').toUpperCase(),
            amount: roundMoney(transaction.paymentAmount || 0),
            reference: transaction.paymentReference || ''
          }],
      subtotal: roundMoney(transaction.subtotal),
      discount: roundMoney(transaction.discountAmount),
      tax: roundMoney(transaction.taxAmount),
