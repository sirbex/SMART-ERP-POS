...
const ProductManagement: React.FC<ProductManagementProps> = ({ onStartPhysicalCount }) => {
  ...
  const handleCreate = async () => {
    ...
    try {
      const productPayload = {
        ...
        taxRate: formData.taxRatePercent ? parseFloat(formData.taxRatePercent) : 0, // store per-product tax (%)
    ...
    try {
      await updateMutation.mutateAsync({
        ...
        request: {
          ...
          taxRate: formData.taxRatePercent ? parseFloat(formData.taxRatePercent) : 0,
          reorderPoint: formData.reorderLevel ? parseInt(formData.reorderLevel) : undefined,
          isActive: formData.isActive,
        },
      });
  ...
  const openEditDialog = (product: ProductItem) => {
    ...
    setFormData({
      ...
      taxRatePercent: ((product as any).taxRate != null ? Number((product as any).taxRate) : 0).toString(),
    });
