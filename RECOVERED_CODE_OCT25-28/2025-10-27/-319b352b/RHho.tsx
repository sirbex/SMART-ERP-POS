...
import type { AlternateUnit } from '@/types/multiUom';
import AlternateUnitsManager from './AlternateUnitsManager';
...
const ProductManagement: React.FC<ProductManagementProps> = ({ onStartPhysicalCount }) => {
  ...
  const handleCreate = async () => {
    ...
    try {
      // Prepare multi-UoM data (backend supports single alternate unit)
      const hasMultipleUnits = formData.alternateUnits.length > 0;
      const firstAlternateUnit = formData.alternateUnits[0];
      ...
      const productPayload = {
        ...
        // Multi-UoM support (backend single unit)
        hasMultipleUnits,
        alternateUnit: hasMultipleUnits ? firstAlternateUnit.unit : undefined,
        conversionFactor: hasMultipleUnits ? firstAlternateUnit.conversionRate : undefined,
      };
  ...
  const openEditDialog = (product: ProductItem) => {
    ...
    // Extract alternate units from product
    const alternateUnits: AlternateUnit[] = [];
    if (product.hasMultipleUnits && product.alternateUnit && product.conversionFactor) {
      alternateUnits.push({
        unit: product.alternateUnit,
        conversionRate: Number(product.conversionFactor),
        barcode: undefined,
      });
    }
              ...
              <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
                ...
                <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                  ...
                  <div className="border-t pt-4 mt-4">
                    <h3 className="font-medium mb-3">Multi-Unit of Measure (Optional)</h3>
                    <p className="text-sm text-muted-foreground mb-4">
                      Add alternate units for this product (e.g., sell in bottles or boxes)
                    </p>
