...
export interface Product {
  ...
  isActive?: boolean;
  is_active?: boolean; // Backend format
  /** If true, this product is tax-exempt (no VAT/sales tax) */
  taxExempt?: boolean;
  isTaxExempt?: boolean; // Alternate naming
  /** Optional per-product tax rate in percent (e.g., 10 for 10%) */
  taxRate?: number;
  hasExpiry?: boolean;
  expiryDate?: string;
  supplierId?: number;
...
export interface SaleItem {
  ...
  quantity: number;
  unit?: string;
  uomId?: string; // UoM ID for new UoM system
  ...
  subtotal?: number;
  tax?: number;
  /** If true, no tax should be applied to this line */
  taxExempt?: boolean;
  /** Optional per-line tax rate (%) captured from product when added */
  taxRate?: number;
  lineTotal?: number;
