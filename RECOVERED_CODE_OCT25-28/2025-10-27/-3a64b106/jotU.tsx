...
const ProductUoMManagement: React.FC<ProductUoMManagementProps> = ({
  ...
  const loadAvailableUoMs = async () => {
    try {
      const response = await api.get('/uoms?limit=100&isActive=true');
      setAvailableUoMs(response.data?.data || []);
    } catch (error) {
  ...
  const loadProductUoMs = async () => {
    ...
    try {
      const response = await api.get(`/products/${product.id}/uoms`);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await api.post(`/products/${product.id}/uoms/bulk`, {
          ...
          <div className="space-y-3">
            ...
            <ScrollArea className="h-[250px] w-full border rounded-lg p-4">
              {productUoMs.length === 0 ? (
                ...
                <div className="space-y-3">
                  {productUoMs.map((pu) => (
                    <div key={pu.uomId} className="border rounded-lg p-4 space-y-3">
