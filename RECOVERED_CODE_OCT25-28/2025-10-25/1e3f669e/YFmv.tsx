...
export function PhysicalCountManagement() {
  ...
  const loadProducts = async () => {
    ...
    try {
      ...
      // Normalize numeric fields coming as strings from backend
...
function PricePopover({ productId }: { productId: string }) {
  ...
  const { data, isFetching, isError } = useQuery({
    queryKey: ['price-quote', productId],
    queryFn: async () => {
      const res = await api.get(`/pricing/retail-wholesale/${productId}`, { params: { quantity: 1 } });
