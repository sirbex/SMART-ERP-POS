...
// ============================================
// Validation Schemas
// - Accept number or numeric string inputs
// - Enforce precision to match DB scales (prices: 2dp, qty: 4dp)
// - Encode business rules (manual vs auto)
// ============================================

// Helpers: coerce to number and validate precision
const money2dp = z.preprocess((v) => (typeof v === 'string' ? Number(v) : v),
  z
    .number({ invalid_type_error: 'Value must be a number' })
    .nonnegative('Value cannot be negative')
    .refine((val) => Number.isFinite(val), 'Value must be finite')
    .refine((val) => val <= 9_999_999_999_999.99, 'Value cannot exceed 9,999,999,999,999.99')
    .refine((val) => {
...
// Discriminated union: auto vs manual
const SetBatchPriceSchema = z.discriminatedUnion('autoPrice', [
  z.object({
    batchId: z.string().cuid('Batch ID must be a valid CUID'),
    autoPrice: z.literal(true),
    manualPrice: z.undefined().optional(),
  }),
  z.object({
    batchId: z.string().cuid('Batch ID must be a valid CUID'),
    autoPrice: z.literal(false),
    manualPrice: money2dp,
  }),
]);
...
const CostChangeAnalysisSchema = z.object({
  productId: z.string().cuid('Product ID must be a valid CUID'),
  newBatchCost: money2dp,
});

// Support both legacy boolean and explicit action enum
const CostChangeDecisionSchema = z.union([
  z.object({
    productId: z.string().cuid('Product ID must be a valid CUID'),
  ...
  z.object({
    productId: z.string().cuid('Product ID must be a valid CUID'),
    action: z.enum(['RECALCULATE', 'SKIP']),
    analysisId: z.string().optional(),
  }),
]);
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const input =
        validatedData.autoPrice === true
          ? {
              batchId: validatedData.batchId,
              manualPrice: undefined,
              useProductFormula: true,
            }
          : {
              batchId: validatedData.batchId,
              manualPrice: validatedData.manualPrice,
              useProductFormula: false,
            };
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const validatedData = CostChangeDecisionSchema.parse(req.body) as any;

      const shouldRecalc =
        typeof validatedData.autoRecalculate === 'boolean'
          ? validatedData.autoRecalculate
          : validatedData.action === 'RECALCULATE';
