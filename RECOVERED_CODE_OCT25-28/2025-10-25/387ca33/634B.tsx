  ...
  const exportPurchaseOrders = async () => {
    try {
      ...
      if (listExportFormat === 'PDF') {
        const res = await api.get('/purchase-orders/export/pdf', { responseType: 'blob', params });
        downloadBlob(res.data, `purchase-orders-${new Date().toISOString().slice(0,10)}.pdf`);
        toast({ title: 'Exported', description: 'Purchase orders exported as PDF.' });
      } else {
  ...
  const exportSelectedOrderItems = async () => {
    try {
      ...
      if (itemsExportFormat === 'PDF') {
        const res = await api.get(`/purchase-orders/${selectedOrderId}/items/export/pdf`, { responseType: 'blob' });
        downloadBlob(res.data, `po-${selectedOrder?.poNumber || selectedOrderId}-items.pdf`);
        toast({ title: 'Exported', description: 'Purchase order items exported as PDF.' });
      } else {
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setFilters({ ...filters, startDate: e.target.value || undefined as any })}
              />
            </div>
            <div>
              ...
              <Input
                ...
                onChange={(e) => setFilters({ ...filters, endDate: e.target.value || undefined as any })}
              />
            </div>
          </div>
        </CardContent>
              ...
              <TableBody>
                {purchaseOrders.map(order => (
                  <TableRow key={order.id}>
                    <TableCell className="font-medium">{order.poNumber || `PO-${order.id}`}</TableCell>
                    <TableCell>{order.supplier?.name ? `Supplier: ${order.supplier.name}` : `Supplier ID: ${order.supplierId}`}</TableCell>
