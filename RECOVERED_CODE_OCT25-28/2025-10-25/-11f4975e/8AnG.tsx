...
const PurchaseReceiving: React.FC = () => {
  ...
  const [grSupplierId, setGrSupplierId] = useState<string>('all');
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2 flex-wrap justify-end">
                <div className="flex items-center gap-2">
                  ...
                  <Input type="date" value={grStartDate} onChange={(e) => setGrStartDate(e.target.value)} className="h-8 w-[140px]" />
                </div>
                <div className="flex items-center gap-2">
                  <Label className="text-xs text-muted-foreground">End</Label>
                  <Input type="date" value={grEndDate} onChange={(e) => setGrEndDate(e.target.value)} className="h-8 w-[140px]" />
                </div>
                <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  onClick={async () => {
                    try {
                      ...
                      const params: any = {};
                      if (grStartDate) params.startDate = grStartDate;
                      if (grEndDate) params.endDate = grEndDate;
                      if (grSupplierId && grSupplierId !== 'all') params.supplierId = grSupplierId;
                      if (grExportFormat === 'PDF') {
                        const res = await api.get('/goods-receipts/export/pdf', { responseType: 'blob', params });
                        const url = window.URL.createObjectURL(res.data);
                        const a = document.createElement('a');
                        a.href = url;
                        a.download = `goods-receipts-${new Date().toISOString().slice(0,10)}.pdf`;
                        a.click();
                        window.URL.revokeObjectURL(url);
                      } else {
                        const res = await api.get('/goods-receipts/export', { responseType: 'blob', params });
                        const url = window.URL.createObjectURL(res.data);
                        const a = document.createElement('a');
                        a.href = url;
