...
export async function createTransaction(transaction: {
  ...
  try {
    ...
    // Determine payment amount: for non-cash (CARD, CREDIT, etc.) default to total
    const computedPaymentAmount = backendMethod === 'CASH'
      ? Number(transaction.paymentAmount ?? 0)
      : Number(transaction.total ?? transaction.paymentAmount ?? 0);

    const backendBody = {
      customerId: transaction.customerId || undefined,
      // Filter out any items with non-positive quantities to avoid server validation errors
    ...
    // Surface server error to caller so UI can show actionable message
    const serverMsg = error?.response?.data?.error || error?.message || 'Unknown error';
    console.error('Error creating transaction via API:', serverMsg, error?.response?.data);
 ...
 */
export async function adjustStock(productId: string, quantity: number, reason?: string): Promise<boolean> {
  try {
    const body = {
      productId: String(productId),
      adjustmentQuantity: Number(quantity),
      reason: reason && reason.trim().length >= 5
        ? reason
        : 'POS quick adjustment to fulfill sale'
    };
    const res = await api.post('/stock-movements/adjustment', body);
    return res.status === 201;
  } catch (error) {
    console.error('Error adjusting stock via API:', error);
    return false;
  }
}
