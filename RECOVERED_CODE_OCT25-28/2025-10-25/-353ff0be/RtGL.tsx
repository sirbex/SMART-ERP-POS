...
const SupplierManagement: React.FC = () => {
  ...
  const [orderItemsExportFormat, setOrderItemsExportFormat] = useState<'CSV' | 'PDF'>('CSV');
  ...
  const exportOrderItems = async () => {
    ...
    try {
      const api = (await import('@/config/api.config')).default;
      const poId = selectedOrder.id;
      if (orderItemsExportFormat === 'PDF') {
        const res = await api.get(`/purchase-orders/${poId}/items/export/pdf`, { responseType: 'blob' });
        downloadBlob(res.data, `po-${selectedOrder.poNumber}-items-${new Date().toISOString().slice(0,10)}.pdf`);
        toast({ title: 'Exported', description: 'Purchase order items exported as PDF.' });
      } else {
        const res = await api.get(`/purchase-orders/${poId}/items/export`, { responseType: 'blob' });
        downloadBlob(res.data, `po-${selectedOrder.poNumber}-items-${new Date().toISOString().slice(0,10)}.csv`);
        ...
        <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Select value={orderItemsExportFormat} onValueChange={(v) => setOrderItemsExportFormat(v as 'CSV' | 'PDF')}>
                  <SelectTrigger className="w-[120px]">
                    <SelectValue placeholder="Format" />
                  </SelectTrigger>
                  <SelectContent>
                ...
                <Button variant="outline" size="sm" onClick={exportOrderItems} disabled={!selectedOrder}>
                  <Download className="h-4 w-4 mr-1" /> Export
                </Button>
              </div>
            </div>
