  ...
  const exportSupplierPerformance = async () => {
    ...
    try {
      if (perfExportFormat === 'PDF') {
        toast({ title: 'PDF export coming soon', description: 'PDF export for supplier performance is not yet available.' });
        return;
      }
      const res = await (await import('@/config/api.config')).default.get(
        `/suppliers/${selectedSupplier.id}/performance/export`,
        { responseType: 'blob' }
      );
  ...
  const exportSupplierHistory = async () => {
    ...
    try {
      if (histExportFormat === 'PDF') {
        toast({ title: 'PDF export coming soon', description: 'PDF export for supplier history is not yet available.' });
        return;
      }
      const res = await (await import('@/config/api.config')).default.get(
        `/suppliers/${selectedSupplier.id}/history/export`,
        { responseType: 'blob' }
      );
