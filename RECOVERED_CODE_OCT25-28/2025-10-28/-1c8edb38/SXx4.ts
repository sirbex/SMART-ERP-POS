...
router.post('/:id/deposit', authenticate, validate(depositValidation), async (req, res, next) => {
  try {
    const { id } = req.params;
    ...
    const result = await prisma.$transaction(async (tx: any) => {
      ...
      // Credit: Customer Deposits Liability (liability increases)
      const { postLedger } = await import('../services/accounting/postLedger.js');
      
      const [cashAccount, depositsAccount] = await Promise.all([
        tx.account.findFirst({ where: { code: 'CASH_IN_HAND' } }),
        tx.account.findFirst({ where: { code: 'CUSTOMER_DEPOSITS' } }),
      ]);
  ...
  async (req, res, next) => {
    try {
      ...
      const creditInfo = {
        ...
        paymentTerms:
          customer.paymentTermsDays > 0 ? `Net ${customer.paymentTermsDays}` : 'Cash on Delivery',
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      logger.info(`Credit limit adjusted for customer ${id}: ${oldLimit} -> ${newCreditLimit}`, {
        userId,
      });
  ...
  async (req, res, next) => {
    try {
      ...
      const totalOutstanding = sales.reduce(
        (sum: number, sale: any) => sum + Number(sale.amountOutstanding),
        0
      );
...
router.post('/:id/payment', authenticate, validate(paymentValidation), async (req, res, next) => {
  try {
    const { id } = req.params;
    ...
    const result = await prisma.$transaction(async (tx: any) => {
          ...
          if (sale && Number(sale.amountOutstanding) > 0) {
            ...
            await tx.sale.update({
              ...
              data: {
                ...
                paymentStatus:
                  paymentForSale >= Number(sale.amountOutstanding) ? 'PAID' : 'PARTIAL',
              },
            });
      ...
      const [cashAccount, arAccount] = await Promise.all([
        tx.account.findFirst({ where: { code: 'CASH_IN_HAND' } }),
        tx.account.findFirst({ where: { code: 'ACCOUNTS_RECEIVABLE' } }),
      ]);
    ...
    try {
      ...
      const detailedSales = sales.map((sale: any) => {
        ...
        const daysOld = Math.floor(
          (now.getTime() - sale.saleDate.getTime()) / (24 * 60 * 60 * 1000)
        );
      ...
      const agingReport = {
        ...
        percentages: {
          current:
            totalOutstanding > 0
              ? ((aging.current / totalOutstanding) * 100).toFixed(2) + '%'
              : '0%',
          days31To60:
            totalOutstanding > 0
              ? ((aging.days30 / totalOutstanding) * 100).toFixed(2) + '%'
              : '0%',
          days61To90:
            totalOutstanding > 0
              ? ((aging.days60 / totalOutstanding) * 100).toFixed(2) + '%'
              : '0%',
          over90Days:
            totalOutstanding > 0
              ? ((aging.days90Plus / totalOutstanding) * 100).toFixed(2) + '%'
              : '0%',
        },
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      logger.info(`Transaction history retrieved for customer ${id}`, {
