...
export interface ProductUoM {
  ...
  enabledForPurchase: boolean;
}

/**
 * Calculate cost per base unit from bulk purchase information
 * @param costPerPurchaseUnit - Total cost for one purchase unit (e.g., UGX 12000 per box)
 * @param quantityPerPurchaseUnit - How many base units in one purchase unit (e.g., 24 bottles per box)
 * @returns Cost per base unit (e.g., UGX 500 per bottle)
 */
...
export function generateSalesUoMOptions(
  ...
  const finalBulkUnitPrice = salesPricing.minimumSellingPrice
    ? Math.max(
        bulkUnitPrice,
        salesPricing.minimumSellingPrice * purchaseInfo.quantityPerPurchaseUnit
      )
    : bulkUnitPrice;
...
export function validatePurchaseInfo(purchaseInfo: PurchaseUoM): void {
  ...
  const calculatedCostPerBase =
    purchaseInfo.costPerPurchaseUnit / purchaseInfo.quantityPerPurchaseUnit;
...
/**
 * Format cost breakdown for display/logging
 * @param purchaseInfo - Purchase UoM information
 * @returns Formatted string showing cost breakdown
 */
export function formatCostBreakdown(purchaseInfo: PurchaseUoM): string {
  return (
    `${purchaseInfo.purchaseUnitName}: UGX ${purchaseInfo.costPerPurchaseUnit.toFixed(2)} ` +
    `(${purchaseInfo.quantityPerPurchaseUnit} units @ UGX ${purchaseInfo.costPerBaseUnit.toFixed(2)} each)`
  );
}
