      ...
      res.status(201).json({
        ...
        transfer: {
          fromAccount: {
            ...
            newBalance: fromAccount!.balance.toString(),
          },
          toAccount: {
            id: toAccount!.id,
            accountNumber: toAccount!.accountNumber,
            newBalance: toAccount!.balance.toString(),
          },
          withdrawal: {
            id: result.debitTransaction.id,
            amount: result.debitTransaction.amount.toString(),
            transactionDate: result.debitTransaction.transactionDate,
          },
          deposit: {
            id: result.creditTransaction.id,
            amount: result.creditTransaction.amount.toString(),
            transactionDate: result.creditTransaction.transactionDate,
          },
        },
      });
    } catch (error: any) {
...
router.put(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // Get updated reconciliation details
      ...
      res.json({
        ...
        reconciliation: {
          id: reconciliation!.id,
          reconciliationNumber: reconciliation!.reconciliationNumber,
          bookBalance: result.closingBalance,
