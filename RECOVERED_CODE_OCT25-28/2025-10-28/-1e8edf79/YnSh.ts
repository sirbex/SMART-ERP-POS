...
// GET /api/suppliers/summary-list - Lightweight list with counts and aggregates for UI grids
router.get(
  '/summary-list',
  authenticate,
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const data = suppliers.map((s: any) => ({
        ...
        currentBalance: s.accountBalance?.toString?.() ?? String(s.accountBalance ?? 0),
        totalPaid: s.totalPaid?.toString?.() ?? String(s.totalPaid ?? 0),
        totalPurchased: s.totalPurchased?.toString?.() ?? String(s.totalPurchased ?? s.totalPurchases ?? 0),
        lastPurchaseDate: s.lastPurchaseDate,
        lastPaymentDate: s.lastPaymentDate,
        // Counts
        orderCount: s._count?.purchaseOrders ?? 0,
        paymentCount: s._count?.supplierPayments ?? 0,
      }));
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (existingSupplier._count.purchaseOrders > 0) {
        ...
        return res.json({
          message:
            'Supplier has purchase orders and cannot be deleted. Supplier has been deactivated instead.',
          supplier,
        });
      }
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const deliveryRate =
        totalOrderedQty > 0 ? ((totalDeliveredQty / totalOrderedQty) * 100).toFixed(2) : '0.00';

      const completionRate =
        totalOrders > 0 ? ((completedOrders / totalOrders) * 100).toFixed(2) : '0.00';
      ...
      logger.info(`Retrieved performance metrics for supplier: ${supplier.name}`, {
    ...
    try {
      ...
      const ordersWithDeliveryDetails = orders.map((po: any) => {
        ...
        const totalOrderedValue = items.reduce(
          (sum: number, item: any) => sum + item.totalOrdered,
          0
        );
      ...
      res.json({
        ...
        pagination: buildPaginationResponse(ordersWithDeliveryDetails, total, { page, limit, skip })
          .pagination,
      });

      logger.info(`Retrieved order history for supplier: ${supplier.name}`, {
        userId: (req as any).user?.id,
      });
    } catch (error) {
      next(error);
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="supplier-performance-${supplier.name.replace(/[^a-z0-9]/gi, '_')}-${Date.now()}.csv"`
      );
      return res.send(csv);
    } catch (error) {
      next(error);
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const orders = await prisma.purchaseOrder.findMany({
        ...
        include: {
          items: {
            include: { product: { select: { name: true, barcode: true, baseUnit: true } } },
          },
        },
        orderBy: { createdAt: 'desc' },
      });
      ...
      for (const po of orders as any[]) {
        for (const item of po.items) {
          ...
          const deliveryStatus =
            received === 0 ? 'NOT_DELIVERED' : received < ordered ? 'PARTIAL' : 'COMPLETE';
          const row = [
            po.poNumber,
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="supplier-history-${supplier.name.replace(/[^a-z0-9]/gi, '_')}-${Date.now()}.csv"`
      );
      return res.send(csv);
    } catch (error) {
      next(error);
          ...
          summarySection: [
            {
              ...
              value: `₱${(totalOrderedValue - totalDeliveredValue).toFixed(2)}`,
              color: '#dc2626',
            },
            {
              label: 'Total Paid:',
              value: `₱${Number(supplier.totalPaid || 0).toFixed(2)}`,
              color: '#059669',
            },
            {
              label: 'Current Balance:',
              value: `₱${Number(supplier.accountBalance || 0).toFixed(2)}`,
              color: '#1e40af',
            },
          ],
        }
