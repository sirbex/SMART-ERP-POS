...
export interface Supplier {
  ...
  currentBalance: Decimal;
  // Aggregate fields (may be omitted on some endpoints)
...
// (Consolidated) Removed duplicated Supplier interface definition below. The Supplier
// interface above now contains all fields required across the app.
...
/**
 * User (matches backend)
 */
export interface User {
  id: string; // CUID in backend
  username: string;
  fullName: string;
  email?: string | null;
...
// ============================================================
// PURCHASE RECEIVING SYSTEM TYPES
// ============================================================
 ...
 */
export type GoodsReceiptStatus = 'DRAFT' | 'FINALIZED';

/**
 * Discrepancy Type
 */
export type DiscrepancyType = 'SHORTAGE' | 'OVERAGE' | 'DAMAGE';

/**
 * Inventory Batch Status
 */
export type InventoryBatchStatus = 'ACTIVE' | 'EXPIRED' | 'RECALLED' | 'DEPLETED';

/**
 * Stock Movement Type
 */
export type StockMovementType = 'PURCHASE_RECEIPT' | 'SALE' | 'ADJUSTMENT' | 'TRANSFER' | 'RETURN';
...
/**
 * Purchase Order (matches Prisma schema)
 */
...
/**
 * Goods Receipt (matches Prisma schema)
 */
...
export interface GoodsReceiptItem {
  id: string; // CUID
  goodsReceiptId: string; // CUID
  purchaseOrderItemId: string; // CUID
  productId: string; // CUID
  product?: Product;
  receivedQuantity: number;
  orderedQuantity: number;
...
/**
 * Inventory Batch (matches Prisma schema)
 */
export interface InventoryBatch {
  ...
  goodsReceiptItemId?: string | null; // CUID
  notes?: string | null;
  createdAt: Date;
  updatedAt: Date;
}
...
/**
 * Stock Movement (matches Prisma schema)
 */
export interface StockMovement {
  ...
  unitCost: Decimal;
  totalCost: Decimal;
  referenceId?: string | null; // CUID
  referenceType?: string | null;
  performedById: string; // CUID
  performedBy?: User;
  notes?: string | null;
  createdAt: Date;
}
...
export interface UpdatePurchaseOrderRequest {
  ...
  notes?: string;
  items?: Array<{
    id?: string; // CUID (for existing items)
    productId: string;
    orderedQuantity: number;
    unitPrice: number;
  }>;
...
/**
 * Finalize Goods Receipt Request
 */
export interface FinalizeGoodsReceiptRequest {
  createBatches?: boolean;
  updateStock?: boolean;
  updatePurchaseOrder?: boolean;
}

/**
 * Update Batch Status Request
 */
...
/**
 * Create Manual Stock Movement Request
 */
...
/**
 * Inventory Batches Query Params
 */
export interface GetInventoryBatchesParams {
  page?: number;
  limit?: number;
  productId?: string;
  status?: InventoryBatchStatus;
  expiringWithinDays?: number;
  sortByExpiry?: 'asc' | 'desc';
}

/**
 * Stock Movements Query Params
 */
...
/**
 * Inventory Batch Statistics
 */
export interface InventoryBatchStats {
  totalBatches: number;
