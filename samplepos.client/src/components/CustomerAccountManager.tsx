...
import type { Customer, CustomerBalance, CustomerTransaction } from '../types/backend';
import type { CreateCustomerData } from '../services/api/customersApi';
import type { DepositRequest, PaymentRequest } from '../services/api/customerAccountsApi';
import { 
  useCustomers, 
  useCustomer,
  useCreateCustomer,
...
import {
  ...
  useMakeDeposit,
  useMakePayment
} from '../services/api/customerAccountsApi';
...
export const CustomerAccountManager: React.FC<CustomerAccountManagerProps> = ({ onClose }) => {
  ...
  // Customer details queries (only fetch when customer is selected)
  const { data: selectedCustomer } = useCustomer(selectedCustomerId || null);
  const { data: customerBalance } = useCustomerBalance(selectedCustomerId || null);
  const { data: transactionsData } = useCustomerTransactions(selectedCustomerId || null);
  
  // Mutations for deposits and payments
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'transactions' | 'payment' | 'deposit'>('overview');
  ...
  const [newCustomerForm, setNewCustomerForm] = useState<CreateCustomerData>({
    name: '',
    phone: '',
    email: '',
    address: '',
  ...
  const handleCreateCustomer = async () => {
    try {
      await createCustomer.mutateAsync({
        ...
        creditLimit: newCustomerForm.creditLimit || undefined,
      ...
      <div className="cam-layout">
        ...
        <div className="cam-sidebar">
          ...
          <div className="cam-customer-list">
            {customersLoading ? (
              <div className="cam-loading">Loading customers...</div>
            ) : filteredCustomers.length === 0 ? (
              <div className="cam-no-results">No customers found</div>
            ) : (
              filteredCustomers.map((customer) => (
                <div
                  key={customer.id}
                  className={`cam-customer-item ${selectedCustomerId === String(customer.id) ? 'active' : ''}`}
                ...
                >
                  ...
                  <div className="cam-customer-info">
                    <span className="cam-customer-phone">{customer.phone || 'N/A'}</span>
                    <span className={`cam-status ${customer.isActive ? 'active' : 'inactive'}`}>
                      {customer.isActive ? 'Active' : 'Inactive'}
                    </span>
                  </div>
                  <div className="cam-customer-balance">
                    Balance: {formatCurrency(Number(customer.currentBalance) || 0)}
                  </div>
                </div>
              ))
            )}
          ...
          {selectedCustomer ? (
            <>
              ...
              <div className="cam-customer-header">
                <div className="cam-customer-details">
                  ...
                  <div className="cam-customer-meta">
                    ...
                    <span>Type: {selectedCustomer.customerType || 'N/A'}</span>
                    <span className={`status ${selectedCustomer.isActive ? 'active' : 'inactive'}`}>
                    ...
                    <div className="cam-summary-card">
                      <div className="cam-balance-info">
                        <div className="cam-current-balance">
                          ...
                          <span className={Number(customerBalance.currentBalance) > 0 ? 'negative' : 'positive'}>
                            {formatCurrency(Number(customerBalance.currentBalance))}
                          </span>
                        </div>
                        <div className="cam-deposit-balance">
                          <label>Deposit Balance</label>
                          <span className="positive">{formatCurrency(Number(customerBalance.depositBalance))}</span>
                        </div>
                        <div className="cam-credit-info">
                          <label>Available Credit</label>
                          <span>{formatCurrency(Number(customerBalance.availableCredit))}</span>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <div className="cam-tab-content">
                {/* Overview Tab */}
                {activeTab === 'overview' && customerBalance && (
                  <div className="cam-overview">
                    <div className="cam-stats-grid">
                      <div className="cam-stat-card">
                        <label>Total Credit Limit</label>
                        <span>{formatCurrency(Number(selectedCustomer.creditLimit) || 0)}</span>
                      </div>
                      <div className="cam-stat-card">
                        <label>Current Balance</label>
                        <span>{formatCurrency(Number(customerBalance.currentBalance))}</span>
                      </div>
                      <div className="cam-stat-card">
                        <label>Available Credit</label>
                        <span>{formatCurrency(Number(customerBalance.availableCredit))}</span>
