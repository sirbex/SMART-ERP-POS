...
const SidebarNav: React.FC<SidebarNavProps> = ({ onSelect, selected, isCollapsed = false }) => {
  ...
  return (
    <aside className={`qb-sidebar fixed left-0 top-0 h-screen pt-16 overflow-y-auto transition-all duration-300 ${isCollapsed ? 'w-20' : 'w-64'}`}>
      <nav className={`flex flex-col gap-1 ${isCollapsed ? 'p-2' : 'p-4'}`}>
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`
                group flex items-center gap-3 ${isCollapsed ? 'px-2 justify-center' : 'px-4'} py-3 rounded-lg text-sm font-medium
            ...
            >
              <span className={`
                ...
                transition-colors duration-200 ${isCollapsed ? '' : 'flex-shrink-0'}
              `}>
                {item.icon}
              </span>
              {!isCollapsed && (
