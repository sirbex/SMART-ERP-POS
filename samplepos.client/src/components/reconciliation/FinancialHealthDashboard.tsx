...
export function FinancialHealthDashboard({ asOfDate }: Props) {
    ...
    const integrityActions = blockedSummaries
        .map((s) => translateRecommendedAction(integrityLane(s)?.recommendedAction ?? null))
        ...
        <div className="mb-6">
                        ...
                        <dd className="text-gray-900 space-y-1">
                            ...
                            {maintenanceIssues.map(({ domainTitle, lane }) => (
                                <p key={`${domainTitle}-cache`} className="text-amber-700 flex items-start gap-1">
                                    ...
                                    {domainTitle}:{' '}
                                    {translateRecommendedAction(lane.recommendedAction)
                                        ?? 'Refresh stored balances for this area'}
                                </p>
                            ))}
                        </dd>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {summaries.map((summary) => {
                    ...
                    const quarantine = summary.lanes.find((l) => l.lane === 'quarantine');
                    const writeoff = summary.lanes.find((l) => l.lane === 'writeoff');
                    ...
                    return (
                        ...
                        >
                            ...
                            {integrity && (
                                <div className="mb-2">
                                    <p className="text-xs text-gray-500">Accounting integrity (period close)</p>
                                    <div className="flex items-center justify-between mt-0.5">
                                        <span className={`text-sm font-medium ${
                                            ...
                                            {toBusinessLabel(laneStatusLabel(integrity))}
                                        </span>
                            ...
                            {cache && (
                                <div className="mb-2">
                                    <p className="text-xs text-gray-500">Stored balances</p>
                                    <p className="text-sm text-gray-700">
                                        {toBusinessLabel(laneStatusLabel(cache))}
                            ...
                            {history && (
                                <div className={quarantine || writeoff ? 'mb-2' : undefined}>
                                    <p className="text-xs text-gray-500">Journal history</p>
                            ...
                            {quarantine && (
                                <div className={writeoff ? 'mb-2' : undefined}>
                                    <p className="text-xs text-gray-500">Quarantine exposure</p>
                                    <p className="text-sm text-gray-700 tabular-nums">
                                        {formatCurrency(quarantine.leftAmount)} still on inventory GL
                                    </p>
                                </div>
                            )}
                            ...
                            {writeoff && (
                                <div>
                                    <p className="text-xs text-gray-500">Overdue / write-off exposure</p>
                                    <p className="text-sm text-gray-700 tabular-nums">
                                        {formatCurrency(writeoff.leftAmount)} overdue ·{' '}
                                        {formatCurrency(writeoff.rightAmount)} written off YTD
                                    </p>
                                </div>
                            )}
                            ...
                            {integrity?.severity === 'critical' && integrity.recommendedAction && (
                                <p className="mt-3 text-xs text-red-700 flex items-start gap-1">
                                    <AlertTriangle className="h-3.5 w-3.5 shrink-0 mt-0.5" />
                                    {translateRecommendedAction(integrity.recommendedAction)
