...
export const ApiTester: React.FC = () => {
  ...
  const checkInventory = async () => {
    ...
    try {
      // Use /products endpoint instead of /inventory
      const response = await api.get('/products');
      const count = response.data?.data?.length || response.data?.pagination?.total || 0;
      setStatus(prev => ({
        ...prev,
        inventory: {
          status: 'connected',
  ...
  const checkTransactions = async () => {
    ...
    try {
      // Use /sales endpoint instead of /transactions
  ...
  const checkAll = async () => {
    ...
    try {
      // Check health first - health endpoint is at root level, not under /api
      ...
      setStatus(prev => ({
        ...
        health: {
          status: 'connected',
          message: healthData.message || 'API is healthy',
