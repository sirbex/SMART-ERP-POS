...
import { marginFromCostAndPrice } from '@/utils/pricing';
...
const ProductManagement: React.FC<ProductManagementProps> = ({ onStartPhysicalCount }) => {
  ...
  const openEditDialog = (product: ProductItem) => {
    ...
    setFormData({
      ...
      taxRatePercent: ((product as any).taxRate != null
        ? Number((product as any).taxRate) * 100
        : 0
      ).toString(), // convert decimal to %
    });
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div className="flex gap-2">
              {onStartPhysicalCount && (
                <Button variant="outline" onClick={onStartPhysicalCount}>
                  <ClipboardList className="h-4 w-4 mr-1" />
                  Physical Count
                </Button>
              )}
              <Button variant="outline" onClick={() => setIsBulkPurchaseDialogOpen(true)}>
                ...
                <TableBody>
                  {filteredProducts.map((product) => (
                    <TableRow key={product.id}>
                      ...
                      <TableCell>{product.baseUnit || 'pcs'}</TableCell>
                      <TableCell className="text-right">
                        <div className="flex flex-col items-end">
                          ...
                          {(product as any).productUoMs &&
                            (product as any).productUoMs.length > 0 && (
                              <span className="text-xs text-muted-foreground">
                                {([...(product as any).productUoMs] as any[])
                                  .filter((pu: any) => Number(pu.conversionFactor) > 1)
                                  .sort(
                                    (a: any, b: any) =>
                                      Number(b.conversionFactor) - Number(a.conversionFactor)
                                  )
                                  .map((pu: any) => {
