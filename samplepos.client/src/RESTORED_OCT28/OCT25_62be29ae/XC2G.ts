...
router.get('/retail-wholesale/:productId', authenticate, async (req: Request, res: Response, next: NextFunction) => {
  ...
  const retailGroup = groups.find((g: any) => g.name.toLowerCase() === 'retail');
  const wholesaleGroup = groups.find((g: any) => g.name.toLowerCase() === 'wholesale');
    ...
    let batches: Array<{ id: string; batchNumber: string; sellingPrice: number | null; retailPrice: number | null; wholesalePrice: number | null; remainingQuantity: number }> | undefined;
    if (includeBatches) {
      const activeBatches = await prisma.inventoryBatch.findMany({
        where: { productId, status: 'ACTIVE', remainingQuantity: { gt: 0 } },
        orderBy: { receivedDate: 'asc' },
        // Do not limit select so we can read newly added fields even before client regenerate
      } as any);
      batches = activeBatches.map((b: any) => ({
        id: b.id,
        batchNumber: b.batchNumber,
        sellingPrice: b.sellingPrice != null ? new Decimal(b.sellingPrice).toNumber() : null,
        retailPrice: b.retailPrice != null ? new Decimal(b.retailPrice).toNumber() : null,
        wholesalePrice: b.wholesalePrice != null ? new Decimal(b.wholesalePrice).toNumber() : null,
