...
export async function recordRepayment(params: {
  ...
  const result = await prisma.$transaction(async (tx: any) => {
    ...
    // Debit: Loan account (reduces loan liability)
    // Credit: Cash/Payment account (reduces cash asset)
    if (loan.accountId) {
      const { postLedger } = await import('../accounting/postLedger.js');
      ...
      // If we have cash account info from payment method, use it
      // Otherwise this will be posted to a default cash/payment account
      // You can extend this to pass cashAccountId as a parameter
      ...
      if (cashAccount) {
        ...
        await postLedger({
          date: paymentDate,
          description: `Loan repayment: ${loan.loanNumber} - ${repaymentNumber}`,
          entries,
          refType: 'loan_repayment',
          refId: repayment.id,
        });
