  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const itemsWithTotals = items.map((item: any) => {
        ...
        return {
          ...item,
          subtotal: new PrismaTypes.Decimal(subtotal),
          discount: new PrismaTypes.Decimal(discount),
          taxRate: new PrismaTypes.Decimal(taxRate),
          taxAmount: new PrismaTypes.Decimal(taxAmount),
          total: new PrismaTypes.Decimal(total),
        };
      });

      const subtotal = itemsWithTotals.reduce(
        (sum: any, item: any) => sum.plus(item.subtotal),
        new PrismaTypes.Decimal(0)
      );
      const totalDiscount = itemsWithTotals.reduce(
        (sum: any, item: any) => sum.plus(item.discount),
        new PrismaTypes.Decimal(0)
      );
      const totalTax = itemsWithTotals.reduce(
        (sum: any, item: any) => sum.plus(item.taxAmount),
        new PrismaTypes.Decimal(0)
      );
      const totalAmount = itemsWithTotals.reduce(
        (sum: any, item: any) => sum.plus(item.total),
        new PrismaTypes.Decimal(0)
      );
      ...
      const invoice = await prisma.$transaction(async (tx: any) => {
        const newInvoice = await tx.invoice.create({
          data: {
            ...
            totalAmount,
            amountPaid: new PrismaTypes.Decimal(0),
            ...
            items: {
              create: itemsWithTotals.map((item: any) => ({
                ...
                quantity: new PrismaTypes.Decimal(item.quantity),
                unit: item.unit,
                unitPrice: new PrismaTypes.Decimal(item.unitPrice),
                taxRate: item.taxRate,
                taxAmount: item.taxAmount,
                discount: item.discount,
                subtotal: item.subtotal,
                total: item.total,
                deliveredQuantity: new PrismaTypes.Decimal(0),
              })),
            },
          },
          include: {
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      res.json({
        ...
        items: invoice.itemsmap((item: any) => ({
          ...item,
          quantity: item.quantity.toString(),
          unitPrice: item.unitPrice.toString(),
          taxRate: item.taxRate.toString(),
      ...
      const result = await prisma.$transaction(async (tx: any) => {
        ...
        const updatedInvoice = await tx.invoice.update({
          ...
          data: {
            ...
            amountOutstanding: isPaid ? new PrismaTypes.Decimal(0) : newAmountOutstanding,
            ...
            entries: [
              {
                ...
                amount: new PrismaTypes.Decimal(paymentAmount),
                type: 'debit' as const,
                currency: 'UGX',
                refType: 'invoice_payment',
                refId: invoice.id,
