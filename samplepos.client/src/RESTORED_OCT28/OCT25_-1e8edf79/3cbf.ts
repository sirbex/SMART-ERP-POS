...
router.get('/:id/performance/export/pdf', authenticate, authorize(['ADMIN', 'MANAGER']), async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    sendTablePdf(
      ...
      {
        summarySection: [
          { label: 'Outstanding Amount:', value: `₱${(totalOrderedValue - totalDeliveredValue).toFixed(2)}`, color: '#dc2626' },
          { label: 'Total Paid:', value: `₱${Number(supplier.totalPaid || 0).toFixed(2)}`, color: '#059669' },
          { label: 'Current Balance:', value: `₱${Number(supplier.accountBalance || 0).toFixed(2)}`, color: '#1e40af' }
        ]
      }
    );
  } catch (error) {
  ...
  try {
    ...
    for (const po of orders as any[]) {
      for (const item of po.items) {
        const ordered = Number(item.orderedQuantity || 0);
        const received = Number(item.receivedQuantity || 0);
        const total = ordered * Number(item.unitPrice || 0);
