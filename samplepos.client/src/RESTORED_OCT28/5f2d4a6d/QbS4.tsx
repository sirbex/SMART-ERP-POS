...
async function fetchDashboardData(): Promise<DashboardData> {
  ...
  salesData.forEach((sale: any) => {
    ...
    items.forEach((item: any) => {
      // Try multiple possible field names for product info
      const productName = item.product?.name || item.productName || item.name || 'Unknown Product';
      const productId = item.productId?.toString() || item.product?.id?.toString() || productName;
      ...
      const price = parseFloat(item.price || item.unitPrice || item.salePrice || 0);
