  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      for (const item of items) {
        ...
        // Accept both semantic units ("base"/"alternate") and display labels (e.g., "PIECE")
        let unitForConversion: string = 'base';
        const incomingUnit = (item as any).unit;
        const hasMulti = Boolean((product as any).hasMultipleUnits);
        if (hasMulti) {
          const baseLabel = String((product as any).baseUnit || '').toLowerCase();
          const altLabel = String((product as any).alternateUnit || '').toLowerCase();
          const u = String(incomingUnit || '').toLowerCase();
          if (!u || u === 'base' || u === baseLabel) {
            unitForConversion = 'base';
          } else if (u === 'alternate' || (altLabel && u === altLabel)) {
            unitForConversion = 'alternate';
          } else {
            // Fallback to base to avoid runtime errors for unknown labels
            unitForConversion = 'base';
          }
        } else {
          unitForConversion = 'base';
