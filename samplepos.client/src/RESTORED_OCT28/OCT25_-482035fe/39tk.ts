  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      for (const item of items) {
        ...
        const providedUnit = (item as any).unit ?? product.baseUnit;
        // Map provided unit label (e.g., 'pcs', 'btl') or key ('base'/'alternate') to converter key
        let unitKey: 'base' | 'alternate' = 'base';
        if (providedUnit === 'base' || providedUnit === (product as any).baseUnit) {
          unitKey = 'base';
        } else if (
          providedUnit === 'alternate' ||
          ((product as any).alternateUnit && providedUnit === (product as any).alternateUnit)
        ) {
          unitKey = 'alternate';
        } else if ((product as any).hasMultipleUnits) {
          return res.status(400).json({
            error: `Invalid unit: ${providedUnit} for product ${product.name}. Allowed: ${(product as any).baseUnit}${(product as any).alternateUnit ? ` or ${(product as any).alternateUnit}` : ''}`
          });
        }
        ...
        // Calculate FIFO cost
        const fifoResult = calculateFIFO((product as any).stockBatches, new Prisma.Decimal(quantityInBaseUnit));
