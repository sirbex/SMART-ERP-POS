  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      for (const item of items) {
        ...
        // Calculate FIFO cost
        const fifoResult = calculateFIFO((product as any).batches, new Prisma.Decimal(quantityInBaseUnit));
      ...
      for (const item of processedItems) {
        ...
        const tax = taxableAmount.mul(item.product.taxAmountRate).div(100);
          ...
          data: {
            ...
            items: {
              create: processedItems.map((item) => ({
                ...
                taxAmount: item.total.mul(item.product.taxRate),
                total: item.total,
              })),
            },
            payments: {
          ...
          if ((item.product as any).trackInventory) {
            ...
            for (const update of batchUpdates) {
              await tx.stockBatch.update({
                where: { id: (update as any).batchId },
                data: { quantityRemaining: { decrement: (update as any).quantityUsed } },
              });
            }
