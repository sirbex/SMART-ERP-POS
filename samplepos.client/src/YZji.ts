...
router.post('/', authenticate, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    for (const item of items) {
      ...
      const uomId: string | null = item.uomId || null;
      ...
      if (item.uomId && product.productUoMs && product.productUoMs.length > 0) {
        ...
        quantityInBaseUnit = new Prisma.Decimal(
          legacyConvertToBaseUnit(product as any, parseFloat(item.quantity), unit)
        );
        actualUnit = unit;
        unitPrice = item.unitPrice ? new Prisma.Decimal(item.unitPrice) : product.sellingPrice;
      }
    ...
    const sale = await prisma.$transaction(async (tx: any) => {
      ...
      for (const item of processedItems) {
        ...
        const batchUpdates = createBatchUpdates(item.fifoResult.allocations);
      ...
      // 1. Record Revenue (Cash/AR + Sales Revenue)
      // 2. Record COGS (COGS + Inventory)
      const { postLedger } = await import('../services/accounting/postLedger.js');
      
      // Find GL accounts
      const [cashAccount, revenueAccount, cogsAccount, inventoryAccount, arAccount] = await Promise.all([
        tx.account.findFirst({ where: { code: 'CASH_IN_HAND' } }),
        tx.account.findFirst({ where: { code: 'SALES_REVENUE' } }),
        tx.account.findFirst({ where: { code: 'COST_OF_GOODS_SOLD' } }),
        tx.account.findFirst({ where: { code: 'INVENTORY' } }),
        tx.account.findFirst({ where: { code: 'ACCOUNTS_RECEIVABLE' } }),
      ]);

      if (cashAccount && revenueAccount && cogsAccount && inventoryAccount) {
        // Determine if this is cash or credit sale
        const isCreditSale = hasCreditPayment;
        const receivableAccount = isCreditSale ? arAccount : cashAccount;

        if (receivableAccount) {
          // Entry 1: Record the sale (Revenue)
          await postLedger({
            date: newSale.saleDate,
            description: `Sale: ${newSale.saleNumber}${customerId ? ` - Customer` : ''}`,
            entries: [
              {
                transactionId: '',
          ...
          const totalCost = processedItems.reduce(
            (sum, item) => sum.add(item.fifoResult.totalCost),
            new Prisma.Decimal(0)
          );

          await postLedger({
            date: newSale.saleDate,
            description: `COGS for sale: ${newSale.saleNumber}`,
            entries: [
              {
                transactionId: '',
