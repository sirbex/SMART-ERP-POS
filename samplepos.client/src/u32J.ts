  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const reconciliation = await startReconciliation({
        ...
        openingBalance: openingBalance ? new Decimal(openingBalance.toString()) : new Decimal(0),
        closingBalance: closingBalance ? new Decimal(closingBalance.toString()) : new Decimal(0),
