...
export async function createBankTransfer(params: {
  ...
  const result = await prisma.$transaction(async (tx: any) => {
    ...
    // Get the accounting account IDs associated with these bank accounts
    const fromGLAccount = await tx.account.findFirst({
      where: { 
        OR: [
          { code: fromAccount.accountNumber },
          { name: { contains: fromAccount.accountName } }
        ]
      },
    });

    const toGLAccount = await tx.account.findFirst({
      where: { 
        OR: [
          { code: toAccount.accountNumber },
          { name: { contains: toAccount.accountName } }
        ]
      },
    });

    // Only post to ledger if both GL accounts are found
    if (fromGLAccount && toGLAccount) {
      ...
      await postLedger({
        ...
        entries: [
          {
            transactionId: '',
            accountId: toGLAccount.id,
            amount,
            type: 'debit' as const,
            currency: 'UGX',
            refType: 'bank_transfer',
            refId: creditTransaction.id,
          },
          {
            transactionId: '',
            accountId: fromGLAccount.id,
            amount,
            type: 'credit' as const,
            currency: 'UGX',
            refType: 'bank_transfer',
            refId: debitTransaction.id,
          },
        ],
        refType: 'bank_transfer',
        refId: reference || `XFER-${Date.now()}`,
      });
    }
...
export async function recordBankTransaction(params: {
  ...
  const transaction = await prisma.$transaction(async (tx: any) => {
    ...
    const bankGLAccount = await tx.account.findFirst({
      where: { 
        OR: [
          ...
          { name: { contains: bankAccount.accountName } }
        ]
      },
    });
    ...
    if (bankGLAccount) {
      ...
      // Determine the contra account based on transaction type
      let contraAccount: any;
      if (type === 'FEE') {
        contraAccount = await tx.account.findFirst({
          where: { code: 'BANK_CHARGES' },
        });
      } else if (type === 'INTEREST') {
        contraAccount = await tx.account.findFirst({
          where: { code: 'INTEREST_INCOME' },
        });
      } else {
        // For DEBIT/CREDIT, use a general cash clearing account
        contraAccount = await tx.account.findFirst({
      ...
      if (contraAccount) {
        ...
        if (isIncrease) {
          // Money coming in: Debit bank, Credit source
          entries.push(
            {
              transactionId: '',
          ...
          // Money going out: Credit bank, Debit expense/destination
          entries.push(
            {
              transactionId: '',
              accountId: contraAccount.id,
...
export async function reconcileTransactions(
  ...
  const closingBalance = reconciledTransactions.reduce(
    (sum: Decimal, tx: any) => sum.plus(new Decimal(tx.amount.toString())),
...
export async function getReconciliationSummary(reconciliationId: string) {
  ...
  const debits = transactions
    .filter((t: any) => new Decimal(t.amount.toString()).lessThan(0))
    .reduce((sum: Decimal, t: any) => sum.plus(new Decimal(t.amount.toString()).abs()), new Decimal(0));

  const credits = transactions
    .filter((t: any) => new Decimal(t.amount.toString()).greaterThan(0))
  ...
  return {
    ...
    summary: {
      ...
      reconciledCount: transactions.filter((t: any) => t.isReconciled).length,
      unreconciledCount: transactions.filter((t: any) => !t.isReconciled).length,
    },
  };
}
