...
export const getSupplier = async (id: string): Promise<SupplierWithHistory> => {
  const { data } = await api.get<ApiResponse<SupplierWithHistory>>(`/suppliers/${id}`);
  return data.data;
};
...
export const createSupplier = async (request: CreateSupplierRequest): Promise<Supplier> => {
  const { data } = await api.post<ApiResponse<Supplier>>('/suppliers', request);
  return data.data;
};
...
export const updateSupplier = async (
  ...
  const { data } = await api.put<ApiResponse<Supplier>>(`/suppliers/${id}`, request);
  return data.data;
};
...
export const getActiveSuppliers = async (): Promise<Supplier[]> => {
  const { data } = await api.get<ApiResponse<Supplier[]>>('/suppliers/active');
  return data.data;
};
