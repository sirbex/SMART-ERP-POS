...
export default function ReconciliationPage() {
    ...
    const readyToClose = laneDataStatus === 'ready'

        && inbox.filter((i) => i.blocksClose).length === 0

        && summaries.length > 0;

    const hero = laneDataStatus === 'ready'

        ? buildWorkspaceHero(inbox, actionQueue, readyToClose)

        : { totalNeedingAttention: 0, blockingCount: 0, warningCount: 0, estimatedMinutes: 0, readyToClose: false, nextAction: null };



    const closeChecklist = laneDataStatus === 'ready'
    ...
    const domainStatuses = laneDataStatus === 'ready'

        ? buildTowerDomainStatuses(summaries, inbox, asOfDate, cashDifference)

        : [];

    const attentionPreview = laneDataStatus === 'ready'

        ? buildTowerAttentionPreview(inbox, asOfDate, 5)

        : [];

    const launchers = laneDataStatus === 'ready'

        ? buildTowerWorkspaceLaunchers(summaries, inbox, asOfDate, cashDifference)
    ...
    return (
        ...
        <div className="p-4 lg:p-6 max-w-7xl mx-auto">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 mb-6">
                ...
                <div className="flex flex-wrap items-end gap-4">
                    ...
                    <button
                        ...
                        disabled={laneDataStatus === 'loading'}
                    ...
                    >
                        ...
                        <RefreshCw className={`h-4 w-4 ${laneDataStatus === 'loading' || isRefreshing ? 'animate-spin' : ''}`} />
            ...
            {laneDataStatus === 'loading' && needsLaneData && (

                <WorkspaceSectionLoading label="Loading control account data…" />
