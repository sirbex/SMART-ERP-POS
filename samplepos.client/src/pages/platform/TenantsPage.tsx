...
function CreateTenantModal({ open, onClose, onCreated }: { open: boolean; onClose: () => void; onCreated: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const res = await platformApi.tenants.create(parsed.data);
      if (res.data.success) { onCreated(); onClose(); }
      else setError(res.data.error || 'Failed to create tenant');
    } catch (err: unknown) {
      setError(getStructuredErrorMessage(err, 'Failed to create tenant'));
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <fieldset className="space-y-3">
            ...
            <div>
              ...
              <input type="password" value={form.ownerPassword} onChange={(e) => handleChange('ownerPassword', e.target.value)} required minLength={8} className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder="Min 8 chars, upper + lower + number" />
              <p className="text-xs text-slate-400 mt-0.5">Must include uppercase, lowercase, and a number.</p>
            </div>
          </fieldset>
        ...
        <div className="w-full max-w-lg bg-white h-full shadow-xl overflow-y-auto" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="flex border-b border-slate-200 overflow-x-auto">
            {drawerTabs.map(({ key, label, icon: Icon }) => (
              <button key={key} onClick={() => setTab(key)} className={`flex items-center gap-1.5 px-3 sm:px-4 py-3 text-sm font-medium transition-colors whitespace-nowrap ${tab === key ? 'border-b-2 border-indigo-600 text-indigo-600' : 'text-slate-500 hover:text-slate-700'}`}>
